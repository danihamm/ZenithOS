...
// Standard VGA 8x16 font - each character is 16 bytes, one byte per row
// Each bit represents a pixel (MSB = leftmost pixel)
const uint8_t FontData[256][16] = {
    // 0x00 - NULL
...
void DrawChar(int x, int y, char c, uint32_t fg, uint32_t bg) {
    const uint8_t* glyph = FontData[(uint8_t)c];
...
void DrawStringTransparent(int x, int y, const char* str, uint32_t fg) {
    ...
    while (*str) {
        if (*str == '\n') {
            ...
            DrawCharTransparent(cursorX, y, *str, fg);
            cursorX += FONT_WIDTH;
        }
        ++str;
    }
